...
public class Bego_3E_Es012b_BallaLaFrescaBoolean {
    public static void main(String[] args) {
        ...
        int[] nG1 = new int[5];
        boolean [] numeriCasuali = new boolean[90];
        boolean ordine = false;

        BoolFalse (numeriCasuali);
        do {
            // Opzioni del menu
            ...
            switch (scelta) {
                case 1:
                    ...
                    for (int i = 0; i < nEstratti.length; i++) {
                        nEstratti[i] = Estrazione(caso, nEstratti, i, numeriCasuali);
    ...
    private static int Estrazione(Random caso, int[] nEstratti, int i, boolean[] numeriCasuali) {
        nEstratti[i] = caso.nextInt(0,90) + 1;

        // Controllo e reimposto il numero in caso di duplicazione
        if (numeriCasuali[nEstratti[i]-1])
        {
            nEstratti[i] = caso.nextInt(0,90) + 1;
        }
        numeriCasuali[nEstratti[i]-1] = true;
    ...
    //funzione per rendere falso il vettore bool
    private static void BoolFalse (boolean[] numeriCasuali)
    {
        for (int i = 0; i < numeriCasuali.length; i++)
        {
            numeriCasuali[i]= false;
        }
    }
