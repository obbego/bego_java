...
public class Main {
    public static void main(String[] args) {
        String[] operazioni = {"VODAFONE",
                ...
                "[5] Elimina contratto",
                "[6] Ricerca scarsaPazzerellaScemetta",
                "[7] Fine"};
        //boolean Sitel=true;
        final int nMax = 3;
        int contrattiVenduti = 0, modContratto = 0, elimContratto = 0, sceltaRicerca, errore;
        String svuotaBuffer;
                ...
                case 6: {
                    if (contrattiVenduti==0)
                        ...
                        if (errore != 0) {
                            ...
                            System.out.println(wrongIndexSearch(random, gestore, contrattiVenduti, sceltaRicerca, keyboard));
                            errore = random.nextInt(2,5)+1;
                        }
                    }
                    break;
    ...
    private static String wrongIndexSearch(Random random, Contatto[] gestore, int contrattiVenduti, int sceltaRicerca, Scanner keyboard) {
        int posSbagliata, j = 0;
        if (sceltaRicerca==2){
            do {
                System.out.println("Inserisci la posizione da cercare:\n Da 1 a " + contrattiVenduti);
                j = keyboard.nextInt()-1;
            } while (j < 0 || j > contrattiVenduti-1);
        ...
        do{
            posSbagliata = random.nextInt(0, contrattiVenduti);
        }
        while (posSbagliata == j);
        return gestore[posSbagliata].stampa();
    }
